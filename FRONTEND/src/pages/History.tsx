  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 relative z-10 pb-24 lg:pb-8">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6 sm:mb-8 pb-4 border-b-2 border-[#006B3C]">
        <div>
          ...
          <p className="font-sans text-xs sm:text-sm text-[#FFFDF5] font-extrabold mt-1 drop-shadow">
            {t('historySub')} · Last {historyList.length}/7 saved
          </p>
        </div>
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-2 bg-[#006B3C] p-1.5 rounded-2xl border border-[#79C968]/40 overflow-x-auto max-w-full">
          ...
          >
            {t('filterAll')} ({historyList.length})
          </button>
          <button
      ...
      {filteredItems.length === 0 ? (
        <div className="bg-[#F7F0DB] border-3 border-[#003622] rounded-2xl p-6 shadow-[4px_6px_0px_#003622] text-[#004E32]">
          <p className="font-mono text-sm font-black">
            {historyList.length === 0
              ? 'No saved queries yet. Ask something on the Ask tab — the newest 7 results are kept in localStorage.'
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="bg-[#006B3C] text-[#FFD400] font-mono text-[10px] font-bold px-2 py-0.5 rounded-full">
                        {(item.performance.guardrailMs || 0) +
                          (item.performance.embeddingMs || 0) +
                          (item.performance.retrievalMs || 0)}
                        ms
                      </span>
                      {isAllowed ? (
                ...
                <div className="flex items-center justify-between pt-3 border-t border-[#003622]/10 font-mono text-xs text-[#006B3C] font-bold">
                  <span>
                    {item.retrieval.chunksRetrieved} {t('chunksCount')}
                  </span>
